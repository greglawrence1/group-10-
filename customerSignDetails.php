<?php
    ...
    if(isset($_POST['subUser'])){
        if($_POST['age'] == ""){
            $error['AgeError'] = "Please enter a age";
            $_SESSION['AgeError'] = "Please enter a age";
        }else{
    ...
    if(empty($error)){
        ...
        trim($password);
        trim($email);
        $query = "INSERT INTO loginCustomer
        (userName, userEmail, userPassword, userAge, firstName, surname, consentForm)
        VALUES
        ('$username', '$email', '$password', '$age', '$firstName', '$surname', '$consent')";

        mysqli_query($connection, $query);
        header("location:registerCustomers.php");
    }else{
